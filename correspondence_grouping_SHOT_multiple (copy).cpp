...
int
loadMCloud (std::string model_filename_)
{
...
int
loadSCloud (std::string scene_filename_)
{
...
int
correspondenceGroup (std::string model_filename_, std::string scene_filename_)
{
  
//  Load clouds
  loadSCloud (scene_filename_);
  loadMCloud (model_filename_);
...
pathIteration()
{
  ...
  { 
    unsigned long s_file_count = 0;
    unsigned long m_file_count = 0;
...
//          DO CORRESPONDENCE GROUPING
            ...
            score[i][j] = correspondenceGroup(model_path.string() + it_m->path().filename().string(), scene_path.string() + it_s->path().filename().string());
